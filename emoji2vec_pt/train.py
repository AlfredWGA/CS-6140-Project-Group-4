...
train_desc_df = load_emoji2vec_dataset(project_path / "emoji2vec_pt" / "data" / "train_desc.txt")
train_kw_df = load_emoji2vec_dataset(project_path / "emoji2vec_pt" / "data" / "train_keyword.txt")
train_df = pd.concat([train_df, train_desc_df, train_kw_df])
...
word2vec = gensim.models.KeyedVectors.load_word2vec_format(project_path / 'pre-trained' / 'GoogleNews-vectors-negative300.bin', binary=True)
...
wandb_logger = WandbLogger(project="emoji2vec_desc_kw", save_dir=current_path / "ckpt")

trainer = pl.Trainer(callbacks=[EarlyStopping(monitor="val_loss", min_delta=0.001)],
